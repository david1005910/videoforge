...
## Phase 9 — macOS Polish (1주) 🔶 (P9-04/05 대기)
...
| P9-04 | Apple Developer ID 발급 + GitHub Secrets 등록                            | M    | —     | ⏳ 외부       |
| P9-05 | CI workflow에 Notarization 활성화 (현재 주석 처리됨)                     | L    | P9-04 | ⏳ P9-04 의존 |
...
## Phase 10 — Update System + Release (3일) 🔶 (P10-05 대기)
...
| P10-05 | CI: notarized DMG 업로드 + Sparkle 호환 RSS      | M    | P9-05  | ⏳ P9-04/05 의존 |

**Phase 10 총 추정**: 약 16h.

---

## Phase 11 — 안정화 / 베타 (2주) 🔶 (P11-01/02/06 대기)

| ID     | 작업                                 | 추정 | 의존 | 상태                 |
| ------ | ------------------------------------ | ---- | ---- | -------------------- |
| P11-01 | 베타 5명 모집 (HBAS 동료 + 유튜버)   | —    | —    | ⏳ 수동              |
| P11-02 | 발견된 버그 일괄 수정 (16h 예약)     | XL   | —    | ⏳ 피드백 의존       |
| P11-03 | 성능 회귀 측정 + 예산 위반 항목 수정 | L    | —    | ✅ (13/13 통과)      |
| P11-04 | 한국어 UX 카피 재검토                | M    | —    | ✅ (143키 검토 완료) |
| P11-05 | 사용자 매뉴얼 (마크다운 + 스크린샷)  | L    | —    | ✅ (19섹션)          |
| P11-06 | 1.0.0 릴리스                         | M    | All  | ⏳ 전체 의존         |
...
v0.2.0 ~ v0.3.0에서 모두 구현 완료.

- ✅ Local Whisper (whisper.cpp x64 빌드, 12 GGML 모델).
- ✅ Bridge 익스텐션 (WebSocket 브릿지 서버, port 9821).
- ✅ Mobile companion (WebSocket remote, 페어링 코드 인증).
- ✅ Cloud 동기화 (Supabase 옵트인, 서비스 스켈레톤).
- ✅ 자체 Veo / Sora 통합 (서비스 스켈레톤, API 출시 대기).
- ✅ 협업 / 공유 라이브러리 (서비스 스켈레톤).
